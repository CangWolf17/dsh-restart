...
/** Windows system paths used by the scheduled-task restart path. */
const POWERSHELL = 'C:\\Windows\\System32\\WindowsPowerShell\\v1.0\\powershell.exe'
const WSCRIPT = 'C:\\Windows\\System32\\wscript.exe'
const CMDEXE = 'C:\\Windows\\System32\\cmd.exe'

/** One-shot scheduled task name for the Windows restart path. */
const RESTART_TASK_NAME = 'dsh-restart'
...
function relaunch() {
  ...
  try {
    ...
    const child = spawn(idx.execPath, argv, { cwd: idx.cwd, detached: process.platform !== 'win32', windowsHide: true, stdio: ['ignore', o, e], env: process.env })
...
function restart(delayMs: number): RestartInfo {
  ...
  if (isWindows) {
    // Windows: sandbox job objects kill child processes when the parent exits
    // (KILL_ON_JOB_CLOSE), so a spawned helper cannot survive the old process's
    // exit. Use a one-shot scheduled task instead: the task runs under Task
    // Scheduler — outside any sandbox job — and performs the relaunch hidden.
    const cmdPath = path.join(os.tmpdir(), `dsh-restart-${stamp}.cmd`)
    const vbsPath = path.join(os.tmpdir(), `dsh-restart-${stamp}.vbs`)
    const port = String(process.env.DSH_WEB_URL || '').match(/:\d+/)?.[0]?.slice(1) || '3080'
    const cmdLine = [process.execPath, ...argv].map(quoteArg).join(' ')
    const cmd = [
      '@echo off',
      `for /f "tokens=5" %%p in ('netstat -ano ^| findstr ":${port}" ^| findstr "LISTENING"') do taskkill /F /PID %%p >nul 2>&1`,
      'ping -n 3 127.0.0.1 >nul',
      `cd /d "${cwd.replace(/"/g, '\\"')}"`,
      `start "" /b ${cmdLine} >> "${logOut}" 2>&1`,
    ...
    if (scheduled.status !== 0) {
      return { ok: false, pid: process.pid, cwd, commandLine: launchCommandLine(), delayMs, logOut, logErr, error: `scheduled-task registration failed (exit ${scheduled.status})` }
    }
    setTimeout(() => process.exit(0), delayMs)
    return { ok: true, pid: process.pid, cwd, commandLine: launchCommandLine(), delayMs, logOut, logErr }
  }
  ...
  const helperCode = [
    ...
    '    const child = spawn(process.execPath, argv, { cwd: cwd, detached: process.platform !== "win32", windowsHide: true, stdio: ["ignore", out, err], env: process.env })',
